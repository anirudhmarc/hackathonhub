...
const ParticipantAuthRedirect: React.FC = () => {
  ...
  useEffect(() => {
  console.log("ParticipantAuthRedirect useEffect:", { isAuthenticated, isLoading, pathname: location.pathname, search: location.search });

    if (isAuthenticated && !isLoading) {
      console.log('ParticipantAuthRedirect: User authenticated, navigating to /dashboard.');
  ...
  sessionStorage.removeItem('auth_processed');
    ...
    if (!isAuthenticated && !isLoading) {
      console.log('ParticipantAuthRedirect: Not authenticated, initiating login redirect.');
    ...
    if (code || state || idTokenHint || postLogoutRedirectUri) {
      console.log('ParticipantAuthRedirect: Cleaning up URL search parameters.');
...
export default ParticipantAuthRedirect;