...
export const ScoringCard = memo(({
  ...
  const loadExistingScore = useCallback(() => {
    if (selectedJudge && getJudgeScoreForTeam && currentStage) {
      ...
      if (previousScore) {
        if (isFinalRound) {
          setInnovation(Math.min(previousScore.innovation, 20));
          setImpact(Math.min(previousScore.impact, 20));
          setTechnicalComplexity(Math.min(previousScore.technical_complexity, 40));
          setPresentation(Math.min(previousScore.presentation, 20));
        } else {
          setInnovation(Math.min(previousScore.innovation, 30));
          setImpact(Math.min(previousScore.impact, 30));
          setTechnicalComplexity(Math.min(previousScore.technical_complexity, 40));
          setPresentation(previousScore.presentation);
        }
    ...
    try {
      ...
      dispatch({
        ...
        payload: {
          ...
          impact,
          presentation: isFinalRound ? presentation : 0,
          feedback: feedback,
  ...
  return (
    <>
      <Card 
        className={`w-full transition-all duration-300 ease-out cursor-pointer hover:shadow-2xl hover:scale-[1.02] hover:bg-gradient-to-br hover:from-white hover:to-blue-50/30 border border-gray-200 hover:border-blue-300 backdrop-blur-sm group`}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="transition-transform duration-300 group-hover:scale-110">
                <TrackIcon track={track as 'student' | 'corporate'} size={36} />
              </div>
                ...
                {effectiveVideoUrl && !isPlaceholderVideo(effectiveVideoUrl) ? (
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    ...
                    <div className="rounded-lg overflow-hidden border border-gray-300 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.01]">
                      <div className="bg-gradient-to-br from-gray-900 to-black w-full" style={{ paddingTop: '56.25%', position: 'relative' }}>
                        <video
                          controls
                          preload="metadata"
                  ...
                  <div className="bg-white rounded-lg p-4 border border-gray-300 transition-all duration-300 hover:border-yellow-400 hover:shadow-lg hover:scale-[1.01] group">
                    <div className="flex justify-between items-center mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-gray-900 transition-colors duration-300 group-hover:text-yellow-700">Novelty</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Input
                          type="number"
                          min={1}
                          max={isFinalRound ? 20 : 30}
                          value={innovation}
                          onChange={handleScoreChange(setInnovation, isFinalRound ? 20 : 30)}
                          onBlur={handleScoreBlur(setInnovation, isFinalRound ? 20 : 30)}
                        ...
                        />
                        <span className="font-bold text-gray-700 transition-colors duration-300 group-hover:text-yellow-700">/{isFinalRound ? 20 : 30}</span>
                    ...
                    <div className="flex justify-between items-center mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        <Input
                          ...
                          max={isFinalRound ? 20 : 30}
                          value={impact}
                          onChange={handleScoreChange(setImpact, isFinalRound ? 20 : 30)}
                          onBlur={handleScoreBlur(setImpact, isFinalRound ? 20 : 30)}
                        ...
                        />
                        <span className="font-bold text-gray-700 transition-colors duration-300 group-hover:text-blue-700">/{isFinalRound ? 20 : 30}</span>
                  ...
                  <div className="bg-white rounded-lg p-4 border border-gray-300 transition-all duration-300 hover:border-purple-400 hover:shadow-lg hover:scale-[1.01] group">
                    <div className="flex justify-between items-center mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-gray-900 transition-colors duration-300 group-hover:text-purple-700">{isFinalRound ? 'Implementation / Technical Excellence' : 'Implementation'}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Input
                          type="number"
                          min={1}
                          max={isFinalRound ? 40 : 40}
                          value={technicalComplexity}
                            onChange={handleScoreChange(setTechnicalComplexity, isFinalRound ? 40 : 40)}
                            onBlur={handleScoreBlur(setTechnicalComplexity, isFinalRound ? 40 : 40)}
                        ...
                        />
                        <span className="font-bold text-gray-700 transition-colors duration-300 group-hover:text-purple-700">/{isFinalRound ? 40 : 40}</span>
                    ...
                    <div className="bg-white rounded-lg p-4 border border-gray-300 transition-all duration-300 hover:border-indigo-400 hover:shadow-lg hover:scale-[1.01] group">
                      <div className="flex justify-between items-center mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-gray-900 transition-colors duration-300 group-hover:text-indigo-700">Presentation Delivery & Pitch Deck</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Input
                            type="number"
                            ...
                            max={20}
                            value={presentation}
                            onChange={handleScoreChange(setPresentation, 20)}
                            onBlur={handleScoreBlur(setPresentation, 20)}
                          ...
                          />
                          <span className="font-bold text-gray-700 transition-colors duration-300 group-hover:text-indigo-700">/20</span>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h5 className="text-sm font-semibold text-gray-900 uppercase tracking-wide">Funding Decision (Optional)</h5>
                  </div>
                    ...
                    <div className="flex-1">
                      <Button
                        type="button"
                        variant={awsFundingVote === 'upvote' ? 'default' : 'outline'}
                        className={`w-full h-12 ${awsFundingVote === 'upvote' ? 'bg-green-600 hover:bg-green-700 border-green-600 shadow-lg scale-105' : 'border-gray-300 hover:border-green-400 hover:bg-green-50 hover:scale-[1.02] hover:shadow-md'} transition-all duration-300 hover:shadow-lg group relative overflow-hidden`}
                        onClick={() => setAwsFundingVote(awsFundingVote === 'upvote' ? null : 'upvote')}
                      >
                        <span className="mr-2 text-lg transition-transform duration-300 group-hover:scale-125">👍</span> 
                    ...
                    <div className="flex-1">
                      <Button
                        type="button"
                        variant={awsFundingVote === 'downvote' ? 'default' : 'outline'}
                        className={`w-full h-12 ${awsFundingVote === 'downvote' ? 'bg-red-600 hover:bg-red-700 border-red-600 shadow-lg scale-105' : 'border-gray-300 hover:border-red-400 hover:bg-red-50 hover:scale-[1.02] hover:shadow-md'} transition-all duration-300 hover:shadow-lg group relative overflow-hidden`}
                        onClick={() => setAwsFundingVote(awsFundingVote === 'downvote' ? null : 'downvote')}
                      >
                        <span className="mr-2 text-lg transition-transform duration-300 group-hover:scale-125">👎</span> 
                    ...
                    <AutoSaveIndicator field="aws_funding_vote" />
                  </div>
                  
                  <div className="bg-white rounded-lg p-3 border border-gray-300">
                    <p className="text-sm text-gray-600 leading-relaxed">
                      Your recommendation will help determine if this project should receive AWS funding for production implementation.
                    </p>
                  </div>
                </div>

                {isFinalRound && track?.toLowerCase().includes('student') && (
                  <div className="space-y-6">
                    <div className="border-b pb-3">
                      <h4 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                        <Award className="h-5 w-5 text-purple-600" />
                        Innovation Excellence Award Recommendation
                      </h4>
                    </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h5 className="text-sm font-semibold text-gray-900 uppercase tracking-wide">Innovation Excellence Award Decision (Optional)</h5>
                    </div>
                    ...
                    <div className="flex gap-3 mb-4 items-center">
                      <div className="flex-1">
                        <Button
                          type="button"
                          variant={awsSpecialAward === true ? 'default' : 'outline'}
                          className={`w-full h-12 ${awsSpecialAward === true ? 'bg-purple-600 hover:bg-purple-700 border-purple-600 shadow-lg scale-105' : 'border-gray-300 hover:border-purple-400 hover:bg-purple-50 hover:scale-[1.02] hover:shadow-md'} transition-all duration-300 hover:shadow-lg group relative overflow-hidden`}
                          onClick={() => setAwsSpecialAward(awsSpecialAward === true ? null : true)}
                        >
                          <span className="mr-2 text-lg transition-transform duration-300 group-hover:scale-125">⭐</span> 
                          <span className="font-semibold">Recommended</span>
                          <div className="absolute inset-0 bg-gradient-to-r from-purple-400/20 to-violet-400/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                        </Button>
                      </div>
                      <div className="flex-1">
                        <Button
                          type="button"
                          variant={awsSpecialAward === false ? 'default' : 'outline'}
                          className={`w-full h-12 ${awsSpecialAward === false ? 'bg-gray-600 hover:bg-gray-700 border-gray-600 shadow-lg scale-105' : 'border-gray-300 hover:border-gray-400 hover:bg-gray-50 hover:scale-[1.02] hover:shadow-md'} transition-all duration-300 hover:shadow-lg group relative overflow-hidden`}
                          onClick={() => setAwsSpecialAward(awsSpecialAward === false ? null : false)}
                        >
                          <span className="mr-2 text-lg transition-transform duration-300 group-hover:scale-125">⚪</span> 
                          <span className="font-semibold">Not Recommended</span>
                          <div className="absolute inset-0 bg-gradient-to-r from-gray-400/20 to-slate-400/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                        </Button>
                      </div>
                      <AutoSaveIndicator field="aws_special_award" />
                    </div>
                    
                    <div className="bg-white rounded-lg p-3 border border-gray-300">
                      <p className="text-sm text-gray-600 leading-relaxed">
                        The <span className="font-semibold text-purple-700">Innovation Excellence Award</span> recognizes outstanding student teams who transform innovative ideas into groundbreaking solutions. This prestigious honor celebrates exceptional creativity, technical excellence, and collaborative achievement in addressing real-world challenges. It represents the highest standard of student innovation, marking recipients as leaders in their field and pioneers of tomorrow's breakthroughs. Your recommendation will help determine if this team deserves this distinguished recognition.
                      </p>
                    </div>
                  </div>
                </div>
