...
const ManageProblemsPage: React.FC = () => {
  ...
  const fetchProblemsAndTracks = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(`${API_GATEWAY_BASE_URL}/admin/problems`, {
        headers: {
      ...
      const apiTracks = response.data.tracks;
    ...
    if (!Array.isArray(apiTracks)) {
      console.error("API response for tracks is not an array:", apiTracks);
      throw new Error("Invalid data format for tracks received from API.");
    }

      
      
      const transformedProblems: Problem[] = apiProblems.map((p: Problem) => ({
          id: p.id,
          title: p.title,
  ...
  const handleAddEditProblem = async () => {
    ...
    try {
      ...
      const url = editingProblem ? `${API_GATEWAY_BASE_URL}/admin/problems/${editingProblem.id}` : `${API_GATEWAY_BASE_URL}/admin/problems`;
      ...
      const payload = {
        ...
        problem_max_slots: formState.max_slots,
        track_id: formState.track_id,
      };
  ...
  setFormState({ title: '', description: '', tag: '', max_slots: 0, track_id: '' });
  ...
  const handleDeleteProblem = async (problemId: string) => {
    ...
    setLoading(true);
    try {
      await axios.delete(`${API_GATEWAY_BASE_URL}/admin/problems/${problemId}`, {
  ...
  track_id: problemToEdit?.trackId || '',
    });
  ...
  }, [idToken, fetchProblemsAndTracks]);
  ...
  if (error) {
    return (
      <div className="aws-container py-8 text-center text-red-600">
        <p className="text-xl font-bold mb-4">Error Loading Problems</p>
        <p>{error}</p>
  ...
  return (
    <div className="aws-container py-8">
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>Problems Summary</CardTitle>
        </CardHeader>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[400px] bg-white shadow-md rounded-lg">
            <DialogHeader>
              ...
              <DialogDescription>
                Fill in the details for the problem statement.
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="tag" value={formState.tag} onChange={(e) => setFormState({ ...formState, tag: e.target.value })} className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="max_slots" className="text-right">
                  Max Slots
                </Label>
                <Input id="max_slots" type="number" value={formState.max_slots} onChange={(e) => setFormState({ ...formState, max_slots: parseInt(e.target.value) || 0 })} className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="track_id" className="text-right">
                  Track
                </Label>
                <select id="track_id" value={formState.track_id} onChange={(e) => setFormState({ ...formState, track_id: e.target.value })} className="col-span-3 p-2 border rounded-md">
      ...
      <Card>
        ...
        <CardContent>
          {problems.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">No problems registered yet.</p>
          ) : (
                ...
                <TableBody>
                  {sortedProblems.map((problem) => (
                    <TableRow key={problem.id}>
                      ...
                      <TableCell>{problem.max_slots}</TableCell>
                      <TableCell>{problem.trackTitle}</TableCell>
                      <TableCell>
