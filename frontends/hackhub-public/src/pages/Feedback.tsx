...
const Feedback = () => {
  ...
  const loadFeedback = useCallback(async () => {
    setLoading(true);
    if (!user?.teamId || !idToken) {
      console.log("Feedback: User has no teamId or idToken, skipping feedback fetch.");
      setLoading(false);
    ...
    try {
      const response = await axios.get(`${API_GATEWAY_BASE_URL}/participant/feedback`, {
        headers: {
  ...
  }, [user?.teamId, idToken, loadFeedback]);

  const [msRemaining, setMsRemaining] = useState<number>(() => Math.max(0, FEEDBACK_RELEASE_DATE.getTime() - Date.now()));

  useEffect(() => {
    const tick = () => {
      setMsRemaining(prev => {
        const next = Math.max(0, FEEDBACK_RELEASE_DATE.getTime() - Date.now());
        return next;
      });
    };
        ...
        <CardContent className="p-12">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Expected by: {formatWithGmt8(FEEDBACK_RELEASE_DATE)}
            </p>
