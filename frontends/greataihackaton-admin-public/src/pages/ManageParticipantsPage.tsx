...
import { Loader2, RefreshCw, PlusCircle, Edit, Trash2, Check, ChevronDown, ChevronUp, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react';
...
const ManageParticipantsPage: React.FC = () => {
  ...
  const fetchParticipants = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`${API_GATEWAY_BASE_URL}/admin/participants`, {
        headers: {
  ...
  const handleAddEditParticipant = async () => {
    ...
    try {
      ...
      const url = editingParticipant
        ? `${API_GATEWAY_BASE_URL}/admin/participants/${editingParticipant.participant_id || editingParticipant.id}`
        : `${API_GATEWAY_BASE_URL}/admin/participants`;
  ...
  const handleDeleteParticipant = async (participantId: string) => {
    ...
    setLoading(true);
    try {
      await axios.delete(`${API_GATEWAY_BASE_URL}/admin/participants/${participantId}`, {
  ...
  const handleApproveParticipant = async (participant: Participant) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${API_GATEWAY_BASE_URL}/admin/participants/${participant.participant_id || participant.id}/approve`, {
  ...
  const totalPeopleCount = studentPeopleCount + corporatePeopleCount;
    ...
    return { 
      ...
      studentTeamsCount: studentTeams.length,
      corporateTeamsCount: corporateTeams.length,
    studentPeopleCount,
  ...
  const filteredParticipants = useMemo(() => {
    ...
    return result;
  }, [participants, trackFilter, searchTerm, sortBy, sortOrder]);
    ...
    <div className="aws-container py-8">
              ...
              {isExpanded && (
                <div className="mt-4 space-y-2 pl-4 border-l-2 border-gray-300">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold">{studentTeamsCount}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span>Corporate Teams:</span>
                    <span className="font-bold">{corporateTeamsCount}</span>
                  </div>
                </div>
              )}
            </div>
              ...
              {isExpanded && (
                <div className="mt-4 space-y-2 pl-4 border-l-2 border-gray-300">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold">{studentPeopleCount}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span>Corporate Participants:</span>
                    <span className="font-bold">{corporatePeopleCount}</span>
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Label htmlFor="track-filter">Filter by Track:</Label>
            <select
              id="track-filter"
              value={trackFilter}
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[425px] bg-white shadow-md rounded-lg">
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="agreeEmail" className="text-right">
                  Agree Emails
                </Label>
                <input type="checkbox" id="agreeEmail" checked={formState.agreeEmailCommunications} onChange={(e) => setFormState({ ...formState, agreeEmailCommunications: e.target.checked })} className="col-span-3 h-4 w-4" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="agreeMarketing" className="text-right">
                  Agree Marketing
                </Label>
                <input type="checkbox" id="agreeMarketing" checked={formState.agreeMarketingEmails} onChange={(e) => setFormState({ ...formState, agreeMarketingEmails: e.target.checked })} className="col-span-3 h-4 w-4" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="agreeTerms" className="text-right">
                  Agree T&C
                </Label>
                <input type="checkbox" id="agreeTerms" checked={formState.agreeTermsConditions} onChange={(e) => setFormState({ ...formState, agreeTermsConditions: e.target.checked })} className="col-span-3 h-4 w-4" />
              </div>
            </div>
            <DialogFooter>
                ...
                <TableBody>
                  {filteredParticipants.map((participant) => (
                    <TableRow key={participant.participant_id || participant.id}>
                      ...
                      <TableCell>{participant.track}</TableCell>
