...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const processAuth = async () => {
      if (authOidc.isAuthenticated && authOidc.user) {
          ...
          try {
              ...
              const response = await axios.get<DashboardData>(`${API_GATEWAY_BASE_URL}/participant/teams`, {
              ...
              if (teamInfo?.hasSubmitted) {
                  try {
                      const submissionResponse = await axios.get(`${API_GATEWAY_BASE_URL}/participant/submissions`, {
                          headers: { 'Authorization': `Bearer ${token}` }
                      });
              ...
              setUser(customUser);
              localStorage.setItem("awsHackHubUser", JSON.stringify(customUser));
              setAvailableTracks(fetchedTracks);

          } catch (error) {
              console.error("AuthContext: Error fetching dashboard data:", error);
              setUser(null);
              localStorage.removeItem("awsHackHubUser");
              setAvailableTracks([]);
              toast({
                  title: "Error",
  ...
  }, [authOidc.isAuthenticated, authOidc.user, authOidc.isLoading, authOidc.error]);
  ...
  const updateUserTeam = (
    ...
    if (authOidc.user) {
        ...
        setUser(updatedUser);
        localStorage.setItem("awsHackHubUser", JSON.stringify(updatedUser));
    } else {
        toast({
