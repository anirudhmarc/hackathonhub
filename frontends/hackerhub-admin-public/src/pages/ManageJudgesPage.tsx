...
const ManageJudgesPage: React.FC = () => {
  ...
  const fetchJudges = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`${API_GATEWAY_BASE_URL}/hackathons/${currentHackathonId}/admin/judges`, {
        headers: {
  ...
  const handleAddEditJudge = async () => {
    ...
    try {
      ...
      const url = editingJudge ? `${API_GATEWAY_BASE_URL}/hackathons/${currentHackathonId}/admin/judges/${editingJudge.judge_id}` : `${API_GATEWAY_BASE_URL}/hackathons/${currentHackathonId}/admin/judges`;
  ...
  const handleBulkAddJudges = async () => {
    ...
    try {
      ...
      const promises = judgesData.map(judgeData => 
        axios.post(`${API_GATEWAY_BASE_URL}/hackathons/${currentHackathonId}/admin/judges`, judgeData, {
          headers: {
            'Authorization': `Bearer ${idToken}`,
            'Content-Type': 'application/json',
          },
  ...
  const handleDeleteJudge = async (judgeId: string) => {
    ...
    setLoading(true);
    try {
      await axios.delete(`${API_GATEWAY_BASE_URL}/hackathons/${currentHackathonId}/admin/judges/${judgeId}`, {
  ...
  }, [idToken, currentHackathonId, fetchJudges]);
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      ...
      try {
        const [assignmentsResp, leaderboardResp] = await Promise.all([
          ...
          axios.get(`${API_GATEWAY_BASE_URL}/hackathons/${currentHackathonId}/admin/leaderboard`, { headers: { Authorization: `Bearer ${idToken}` } }),
        ]);
    ...
    fetchSummary();
  }, [idToken, currentHackathonId, judges]);
