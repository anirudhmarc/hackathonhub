...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const base = `${API_URL}/hackathons/${currentHackathonId}/admin`;
    Promise.allSettled([
      axios.get(`${base}/participants`, { headers }),
      axios.get(`${base}/teams`, { headers }),
      axios.get(`${base}/judges`, { headers }),
      axios.get(`${base}/problems`, { headers }),
      axios.get(`${base}/assignments`, { headers }),
    ...
    <div className="w-full max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-500">
          {currentHackathon ? `Overview for ${currentHackathon.name}` : 'Hackathon management overview'}
        </p>
      </div>

      {!currentHackathonId && (
        <div className="mb-6 rounded-lg border border-indigo-200 bg-indigo-50 p-4 text-sm text-indigo-800 flex items-center justify-between gap-4">
          <span>Select a hackathon from the top bar, or create one to start managing it.</span>
          <Link
            to="/hackathons"
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <QuickLink label="Manage Teams" href="/teams" description="View and manage registered teams" />
          <QuickLink label="Manage Judges" href="/judges" description="Add judges and track progress" />
          <QuickLink label="Agenda" href="/problems" description="Create and edit the event agenda" />
          <QuickLink label="Judging Rubric" href="/rubric" description="Edit scoring criteria and weights" />
