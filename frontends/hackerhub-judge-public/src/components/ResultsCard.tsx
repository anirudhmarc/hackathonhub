...
export const ResultsCard = ({ team, rank, stageId }: ResultsCardProps) => {
  ...
  return (
    <>
      <Card
        className={`cursor-pointer hover:shadow-xl hover:bg-accent/40 transition-all duration-300 ease-out hover:scale-[1.02] hover:-translate-y-1 group
          ${isTopThree ? 'border-2 shadow-xl' : 'hover:border-primary/50'}
          ${rank === 1 ? 'border-yellow-500 bg-yellow-50 shadow-yellow-100' : ''}
          ${rank === 2 ? 'border-gray-400 bg-gray-50 shadow-gray-100' : ''}
          ${rank === 3 ? 'border-amber-700 bg-amber-50 shadow-amber-100' : ''}`}
                ...
                <div className="space-y-4">
                  {teamScores.length > 0 ? (
                    teamScores.map((score) => (
                      <div key={score.judgeId} className="bg-muted/40 p-4 rounded-lg border border-muted transition-all duration-300 hover:shadow-lg hover:scale-[1.01] hover:border-primary/20 group">
                        ...
                        <div className={`grid ${isFinalRound ? 'grid-cols-2' : 'grid-cols-2'} gap-x-4 gap-y-3 text-sm`}>
                          <div className="flex justify-between items-center p-2 bg-white/50 rounded-md transition-all duration-200 hover:bg-white/70">
                            <span className="flex items-center gap-2">💡 <span>Innovation:</span></span>
                            <span className="font-semibold text-primary">{score.innovation}/10</span>
                          </div>
                          <div className="flex justify-between items-center p-2 bg-white/50 rounded-md transition-all duration-200 hover:bg-white/70">
                            <span className="flex items-center gap-2">🛠️ <span>Feasibility:</span></span>
                            <span className="font-semibold text-primary">{score.technicalComplexity}/10</span>
                          </div>
                          <div className="flex justify-between items-center p-2 bg-white/50 rounded-md transition-all duration-200 hover:bg-white/70">
                            <span className="flex items-center gap-2">📈 <span>Impact:</span></span>
                            <span className="font-semibold text-primary">{score.impact}/10</span>
                          </div>
                          <div className="flex justify-between items-center p-2 bg-white/50 rounded-md transition-all duration-200 hover:bg-white/70">
                            <span className="flex items-center gap-2">✅ <span>Working Solution:</span></span>
                            <span className="font-semibold text-primary">{score.presentation}/10</span>
                          </div>
                        </div>
