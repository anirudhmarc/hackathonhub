...
const ManageTeamsPage: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'leader_email' | 'track' | 'problem_title' | 'has_submitted' | 'is_finalist' | null>('name');
  ...
  const fetchTeams = useCallback(async () => {
    ...
    try {
      // Fetch teams data
      const teamsResponse = await axios.get(`${API_GATEWAY_BASE_URL}/admin/teams`, {
        headers: {
      ...
      setTracks(teamsResponse.data.tracks);

      // Fetch participants data from DynamoDB
      try {
        const participantsResponse = await axios.get(`${API_GATEWAY_BASE_URL}/admin/participants`, {
          headers: {
            'Authorization': `Bearer ${idToken}`,
          },
        });
  ...
  const handleAddEditTeam = async () => {
    ...
    try {
      ...
      const url = editingTeam ? `${API_GATEWAY_BASE_URL}/admin/teams/${editingTeam.id}` : `${API_GATEWAY_BASE_URL}/admin/teams`;
  ...
  const handleDeleteTeam = async (teamId: string) => {
    ...
    setLoading(true);
    try {
      await axios.delete(`${API_GATEWAY_BASE_URL}/admin/teams/${teamId}`, {
  ...
  const handleSetFinalists = async () => {
    ...
    try {
      ...
      const updatePromises = allTeamIds.map(async teamId => {
        ...
        await axios.put(`${API_GATEWAY_BASE_URL}/admin/teams/${teamId}`, {
  ...
  useEffect(() => {
    if (editingTeam) {
      ...
      const trackId = tracks.find(t => t.id === editingTeam.track_id)?.id || '';
  ...
  const filteredFinalistTeams = useMemo(() => {
    ...
    return teams.filter(team => 
      ...
      team.track.toLowerCase().includes(searchLower)
    );
  }, [teams, finalistSearchTerm]);

  const { totalTeams, finalistTeamsCount, problemStatementStats, submissionStats, trackStats, participantStats } = useMemo(() => {
  ...
  const studentTeams = teams.filter(t => t.track === 'student').length;
  const corporateTeams = teams.filter(t => t.track === 'corporate').length;
  
  // Calculate actual participant counts from DynamoDB data (only APPROVED)
  const studentParticipants = participants
    .filter(p => p.track.toLowerCase() === 'student' && p.status === 'APPROVED')
    .reduce((sum, p) => sum + (p.memberCount || 0), 0);
  
  const corporateParticipants = participants
    .filter(p => p.track.toLowerCase() === 'corporate' && p.status === 'APPROVED')
    .reduce((sum, p) => sum + (p.memberCount || 0), 0);
  
  const totalParticipants = studentParticipants + corporateParticipants;
    ...
    return { 
      ...
      trackStats: {
        student: studentTeams,
        corporate: corporateTeams
      },
      participantStats: {
        student: studentParticipants,
        corporate: corporateParticipants,
  ...
  const filteredTeams = useMemo(() => {
    let result = teams.filter(team => {
      const matchesTrack = trackFilter === 'all' || team.track_id === trackFilter;
      const matchesProblem = problemFilter === 'all' || team.problem_id === problemFilter;
      return matchesTrack && matchesProblem;
    });
    ...
    return result;
  }, [teams, trackFilter, problemFilter, searchTerm, sortBy, sortOrder]);

  const handleSort = (column: 'name' | 'leader_email' | 'track' | 'problem_title' | 'has_submitted' | 'is_finalist') => {
    if (sortBy === column) {
  ...
  <h1 className="text-3xl font-bold mb-6">Manage Teams</h1>
      <Card className="mb-6">
        ...
        <CardContent className="flex flex-col gap-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex-1 flex flex-col p-4 bg-muted/50 rounded-md">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  {showTeamBreakdown ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                </div>
              </div>
              {showTeamBreakdown && (
                <div className="mt-4 space-y-2 pl-4 border-l-2 border-gray-300">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold">{trackStats.student}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span>Corporate Teams:</span>
                    <span className="font-bold">{trackStats.corporate}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            {/* Total Participants section hidden */}
            {false && <div className="flex-1 flex flex-col p-4 bg-muted/50 rounded-md">
              <div 
                className="flex items-center justify-between cursor-pointer"
                onClick={() => setShowParticipantBreakdown(!showParticipantBreakdown)}
              >
                <h3 className="font-medium text-lg">Total Participants:</h3>
                <div className="flex items-center gap-2">
                  <span className="text-2xl font-bold">{participantStats.total}</span>
                  {showParticipantBreakdown ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                </div>
              </div>
              {showParticipantBreakdown && (
                <div className="mt-4 space-y-2 pl-4 border-l-2 border-gray-300">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold">{participantStats.student}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span>Corporate Participants:</span>
                    <span className="font-bold">{participantStats.corporate}</span>
                  </div>
                </div>
              )}
            </div>}
      ...
      <div className="flex justify-between items-center mb-4 gap-4">
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            <Label htmlFor="track-filter" className="whitespace-nowrap">Track:</Label>
            <select
              id="track-filter"
              value={trackFilter}
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="trackId" className="text-right">Track</Label>
                <select id="trackId" value={formState.trackId} onChange={handleTrackChange} className="col-span-3 p-2 border rounded-md">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <select id="problemId" value={formState.problemId} onChange={(e) => setFormState({ ...formState, problemId: e.target.value })} className="col-span-3 p-2 border rounded-md">
                  ...
                  {problems
                    .filter(problem => !formState.trackId || problem.trackId === formState.trackId)
                    .map(problem => (
                ...
                <TableBody>
                  {filteredTeams.map((team) => (
                    <TableRow key={team.id}>
                      ...
                      <TableCell className="capitalize">{team.track}</TableCell>
