...
export const HackathonProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const shouldMakeViewingApiCalls = isAuthenticated;
  const shouldMakeScoresApiCalls = isAuthenticated && !!state.currentStage?.stage_id;
  ...
  const refreshData = useCallback(() => {
    ...
    if (!shouldLockLeaderboard) {
      if (state.currentStage) {
        localStorage.removeItem(`cache_scores-${state.currentStage.stage_id}`);
        localStorage.removeItem(`cache_time_scores-${state.currentStage.stage_id}`);
      }
      scoresApi.refetch();
    }
  ...
  const refreshProblemStatements = useCallback(() => {
    ...
    localStorage.removeItem('cache_problemStatements');
    localStorage.removeItem('cache_time_problemStatements');
    problemStatementsApi.refetch();
